...
void EvaluateTradeSignal(int current_bar_i, datetime t, double live_price, int trigger_dir, double p_pct, bool is_strong, double minor_extreme_sl, int maj_extreme_i, bool is_second_trade = false, bool is_test = false)
  {
   ...
   if (total_points >= InpMinTradeScoreLimit) {
       ...
       if (maj_extreme_i != last_broadcast_maj_extreme_i || is_test || maj_extreme_i == 0 || is_second_trade) {
...
void ProcessBar(int i, const double &open[], const double &high[], const double &low[], const double &close[], const datetime &time[], SState &state, bool is_history, bool draw_ui = true)
  {
   ...
   if (!is_history && g_pending_active && i > g_pending_bar_i) {
       ...
       if (i - g_pending_bar_i > InpRetestMaxBars) {
           ...
           if (executed) {
               EvaluateTradeSignal(i, time[i], g_pending_entry, g_pending_dir, g_pending_p_pct, g_pending_is_strong, g_pending_sl, g_pending_maj_extreme_i, g_pending_is_second_trade, false);
   ...
   bool in_pullback_zone = (p_pct >= InpMinPullbackPct); // Maksimum %100 limitini kaldirdik ki buyuk sweep'lerde tracking durmasin.
   ...
   if(virtual_prev == 0)
     {
      ...
      if (InpTestTradeExecution) {
          ...
          EvaluateTradeSignal(rates_total-1, TimeCurrent(), SymbolInfoDouble(Symbol(), SYMBOL_BID), test_dir, live_pct, true, dummy_ext, 0, false, true);
      }
     }
   else
     {
