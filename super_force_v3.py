...
# 1. FIND THE MANUAL TEST BLOCK
manual_test_start = text.find('// --- 🧪 MANUEL TEST SİNYALİ FIRLATICI ---')
manual_test_end = text.find('return(INIT_SUCCEEDED);', manual_test_start)

manual_code = """// --- 🧪 MANUEL TEST SİNYALİ FIRLATICI ---
   ...
   """
text = text[:manual_test_start] + manual_code + text[manual_test_end:]

# 2. FIND THE AUTO TRADE BLOCK
auto_trade_start = text.find('// --- YENİ AKILLI JSON AUTO-TRADE YAZICI ---')
auto_trade_end = text.find('       } else {\n           string reason = (!is_deep_elastic)', auto_trade_start)

auto_trade_code = """// --- YENİ AKILLI JSON AUTO-TRADE YAZICI ---
...
"""
text = text[:auto_trade_start] + auto_trade_code + text[auto_trade_end:]
