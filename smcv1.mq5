      ...
      if(val_l < old_trig)
        {
         ...
         if (state.choch_dir == -1 && state.t2_h != 0) {
             // Instead of resetting, we update T2 to the new high. This allows tracking sweeps (2nd trade logic).
      ...
      if(val_h > old_trig)
        {
         ...
         if (state.choch_dir == 1 && state.t2_l != 0) {
             // Instead of resetting, we update T2 to the new low. This allows tracking sweeps (2nd trade logic).
      ...
      if (should_eval_bear && t2_valid) {
          ...
          if (draw_ui && (!is_history || (InpWaitRetest && is_just_closed))) {
              ...
              if (state.d1_i != last_alert_d1_i_bear) {
                  // Düşüş yönü: T2, T1'in üstüne çıkarsa (likidite alırsa) 2. İşlem olur. Altında kalırsa 1. İşlem olur.
                  bool is_second_trade = (state.t2_h > state.t1_h);
                  bool is_first_trade = !is_second_trade;
                  ...
                  if (should_execute) {
                      if (is_second_trade) Alert("2. İşlem");
      ...
      if (should_eval_bull && t2_valid) {
          ...
          if (draw_ui && (!is_history || (InpWaitRetest && is_just_closed))) {
              ...
              if (state.d1_i != last_alert_d1_i_bull) {
                  // Yükseliş yönü: T2, T1'in altına inerse (likidite alırsa) 2. İşlem olur. Üstünde kalırsa 1. İşlem olur.
                  bool is_second_trade = (state.t2_l < state.t1_l);
                  bool is_first_trade = !is_second_trade;
