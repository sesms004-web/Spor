...
void ShdBxUpdateStats(double h,double l,double c,double prev_c,datetime bar_time)
{
   for(int k=0;k<g_shd_cnt;k++){
      ...
      if(ts==0){if(im){g_shd_appr[k]=(na!=0)?na:(c>(top+bot)/2.0?1:-1);g_shd_touch[k]=1;g_shd_cnt_in[k]=1;g_shd_ev_t[k]=bar_time;}}
         ...
         }else if(im){g_shd_cnt_in[k]++;g_shd_ev_t[k]=bar_time;}
      }
...
void CheckBaseDropBox(int i, const double &open[], const double &high[], const double &low[], const double &close[], const datetime &time[], SState &state, string pfx)
{
   ...
   if(cur_body > avg_body * InpBigCandleMult) {
      ...
      if(base_count > 0) {
         ...
         string nm = GetUniqueName(pfx + "Box_OB_");
         string wk_abv = GetUniqueName(pfx + "Box_OB_WkAbv_");
         string wk_blw = GetUniqueName(pfx + "Box_OB_WkBlw_");
                ...
                const double &close[],const datetime &time[],SState &state,bool is_history)
{
   ...
   if(state.min_tr==1){
      ...
      if(val_l<ot){
         ...
         if(InpShowMin)DrawLine(GetCycleName(pfx+"Minor_", 2),GetTimeSafe(time,si),sp,GetTimeSafe(time,pi),pp,InpColorMin,1,STYLE_SOLID);
      ...
      if(val_h>ot){
         ...
         if(InpShowMin)DrawLine(GetCycleName(pfx+"Minor_", 2),GetTimeSafe(time,si),sp,GetTimeSafe(time,ti),tp,InpColorMin,1,STYLE_SOLID);
   ...
   if(state.maj_tr==1){
      ...
      if(state.maj_st==0){
         ...
         if(act!=EMPTY_VALUE&&val_l<act){
            ...
            if(InpShowMaj)DrawLine(GetCycleName(pfx+"Major_", 2),GetTimeSafe(time,state.anc_i),state.anc_v,GetTimeSafe(time,state.maj_h_i),state.maj_h,InpColorBull,2,STYLE_SOLID);
         ...
         if(state.maj_l!=EMPTY_VALUE&&state.maj_l!=0&&val_c<state.maj_l){
            ...
            if(InpShowMaj)DrawLine(GetCycleName(pfx+"Major_", 2),GetTimeSafe(time,state.anc_i),state.anc_v,GetTimeSafe(time,state.tmp_h_i),state.tmp_h,InpColorBull,2,STYLE_SOLID);
         ...
         if(val_c>state.maj_h){
            ...
            if(InpShowMaj)DrawLine(GetCycleName(pfx+"Major_", 2),GetTimeSafe(time,state.anc_i),state.anc_v,GetTimeSafe(time,state.maj_l_i),state.maj_l,InpColorBull,2,STYLE_SOLID);
      ...
      if(state.maj_st==0){
         ...
         if(act!=EMPTY_VALUE&&val_h>act){
            ...
            if(InpShowMaj)DrawLine(GetCycleName(pfx+"Major_", 2),GetTimeSafe(time,state.anc_i),state.anc_v,GetTimeSafe(time,state.maj_l_i),state.maj_l,InpColorBear,2,STYLE_SOLID);
         ...
         if(state.maj_h!=EMPTY_VALUE&&state.maj_h!=0&&val_c>state.maj_h){
            ...
            state.maj_tr=1;state.maj_st=0;state.bos_i=i;state.bos_count=0;
            if(InpShowMaj)DrawLine(GetCycleName(pfx+"Major_", 2),GetTimeSafe(time,state.anc_i),state.anc_v,GetTimeSafe(time,state.tmp_l_i),state.tmp_l,InpColorBear,2,STYLE_SOLID);
         ...
         if(state.maj_l!=EMPTY_VALUE&&val_c<state.maj_l){
            ...
            if(InpShowMaj)DrawLine(GetCycleName(pfx+"Major_", 2),GetTimeSafe(time,state.anc_i),state.anc_v,GetTimeSafe(time,state.maj_h_i),state.maj_h,InpColorBear,2,STYLE_SOLID);
