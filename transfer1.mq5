...
void EvaluateTradeSignal(int current_bar_i, datetime t, double live_price, int trigger_dir, double p_pct, bool is_strong, double ext_pt)
  {
   ...
   string stats_h1 = "[Maks:%" + DoubleToString(mp_h1, 1) + " | Anlık:%" + DoubleToString(p_h1, 1) + "]\n";
   ...
   if (h1_mom_15) {
       ...
       if (is_h1_aligned) {
           if (h1_is_premium) { h1_points = 30; h1_text = h1_dir_emoji + " " + stats_h1 + "  └ 🎯 %50 Üstü Yön Uyumlu [+30]\n"; }
           else               { h1_points = 10; h1_text = h1_dir_emoji + " " + stats_h1 + "  └ ⚠️ %50 Altı Yön Uyumlu [+10]\n"; }
       } else {
           if (h1_is_premium) { h1_points = 20; h1_text = h1_dir_emoji + " " + stats_h1 + "  └ 📉 %50 Üstü Ters Yön [+20]\n"; }
           else               { h1_points = 30; h1_text = h1_dir_emoji + " " + stats_h1 + "  └ 📉 %50 Altı Ters Yön [+30]\n"; }
       }
   }
   ...
   if(InpAlertPush) {
       string push_msg = "🚨 [" + Symbol() + "] " + dir_str + " | " + verdict + " (" + IntegerToString(total_points) + ")\n";
       push_msg += "M1:%" + DoubleToString(p_m1, 1) + " H1:%" + DoubleToString(p_h1, 1) + " M30:%" + DoubleToString(p_m30, 1) + "\n";
       push_msg += "M15:%" + DoubleToString(p_m15, 1) + " M5:%" + DoubleToString(p_m5, 1);
