...
// Shadow mode (TF analizi için - grafik nesnesi çizmeden)
...
{
    // Shadow mode: sadece shadow dizisine yaz
    if(g_shadow_mode){
        ...
        if(val_c<state.d1_l){
            if(!tv){state.choch_dir=0;state.d1_l=0;state.t2_h=0;}
                ...
                if(draw_ui){
                    ...
                    if(!g_shadow_mode&&isn&&tdx>=0&&InpShowChoch){
                    ...
                    // --- Bearish Kutu Kontrolu & Bildirimi ---
                    if(!g_shadow_mode && isn && tdx >= 0 && (tdx == 0 || tdx == 1)) {
                        int first_k = -1;
                        for(int k=0; k<g_bx_cnt; k++) {
                            if(g_bx_state[k] > 0 && g_bx_touch_state[k] > 0) { first_k = k; break; }
                        }
                        if(first_k >= 0) {
                            string stat1 = GetBoxStatusStr(g_bx_touch_state[first_k], g_bx_approach[first_k]);
                            bool supports_down = (StringFind(stat1, "Asagi") >= 0 || StringFind(stat1, "Alti Deldi") >= 0 || StringFind(stat1, "Alttan Girdi") >= 0);
                            if(supports_down) {
                                string msg = BuildMTFNotification(-1, tdx, "1. Kutu: " + stat1);
                                if(!is_history) {
                                    if(InpAlertPopup) Alert(msg);
                                    if(InpAlertPush) SendNotification(msg);
                                }
                                if(tn != "") ObjectSetString(0,tn,OBJPROP_TEXT,IntegerToString(tdx+1) + "W");
                            }
                        }
                    }

                    if(!g_shadow_mode && isn)ld1b=state.d1_i;
                }
                state.choch_dir=0;
            }
        }
        ...
        if(val_c>state.d1_h){
            if(!tv){state.choch_dir=0;state.d1_h=0;state.t2_l=0;}
                ...
                if(draw_ui){
                    ...
                    // --- Bullish Kutu Kontrolu & Bildirimi ---
                    if(!g_shadow_mode && isn && tdx >= 0 && (tdx == 0 || tdx == 1)) {
                        int first_k = -1;
                        for(int k=0; k<g_bx_cnt; k++) {
                            if(g_bx_state[k] > 0 && g_bx_touch_state[k] > 0) { first_k = k; break; }
                        ...
                        if(first_k >= 0) {
                            string stat1 = GetBoxStatusStr(g_bx_touch_state[first_k], g_bx_approach[first_k]);
                            bool supports_up = (StringFind(stat1, "Yukari") >= 0 || StringFind(stat1, "Yukari Deldi") >= 0 || StringFind(stat1, "Ustten Girdi") >= 0);
                            if(supports_up) {
                                string msg = BuildMTFNotification(1, tdx, "1. Kutu: " + stat1);
                                if(!is_history) {
                                    if(InpAlertPopup) Alert(msg);
                    ...
                    if(!g_shadow_mode && isn)ld1l=state.d1_i;
                }
                state.choch_dir=0;
            }
        }
...
string AnalyzeTFBoxes(ENUM_TIMEFRAMES tf, double days_inp)
{
    ...
    if(!il)ProcessBar(li,o,h,l,c,t,sc,true,true); // shadow: is_history=true, faint kutular da ateslensin
    else{double pc=(li>0)?c[li-1]:c[li];ShdBxUpdateStats(h[li],l[li],c[li],pc,t[li]);}

    g_shadow_mode=false;

    // En son etkileşimli kutuyu bul (once dokunulmus olan, yoksa herhangi aktif)
    int bk=-1; datetime bt=0;
    for(int k=0;k<g_shd_cnt;k++){
        if(g_shd_state[k]==0)continue;
        if(g_shd_touch[k]>0 && g_shd_ev_t[k]>bt){bt=g_shd_ev_t[k];bk=k;}
    }
    // Hic dokunulmus yoksa son aktif kutuyu al
    if(bk<0){
...
string GetBoxStatusStr(int ts, int appr)
{
    ...
    if(ts==3)return (appr==1)?"Ustten Geldi - Icinden Tepki Yukari":"Alttan Geldi - Icinden Tepki Asagi";
    return "?";
}

// ─── MTF Bildirim Olusturucu ─────────────────────────────────────────
string BuildMTFNotification(int direction, int tdx, string res_detail)
{
    string msg = "";
    string header = "";
    if (direction == -1) header = "🔴 === ISLEME DAHIL OLABILIR (YON ASAGI) === 🔴\n";
    else if (direction == 1) header = "🟢 === ISLEME DAHIL OLABILIR (YON YUKARI) === 🟢\n";
    else header = "🧪 === ISLEME DAHIL OLABILIR (TEST) === 🧪\n";

    msg += header;
    msg += Symbol() + " | " + EnumToString(Period()) + "\n";
    if (direction == 0) msg += "CHoCH Sinyali: TEST Sinyali\n\n";
    else msg += "CHoCH Sinyali: " + IntegerToString(tdx+1) + ". Sinyal\n\n";

    msg += "--- 🌐 MTF KUTU ANALIZI ---\n";

    ENUM_TIMEFRAMES tfs[] = {PERIOD_M15, PERIOD_M30, PERIOD_H1, PERIOD_H4, PERIOD_D1};
    for (int i=0; i<5; i++) {
        string tf_res = AnalyzeTFBoxes(tfs[i], GetDaysForTF(tfs[i]));

        string lbl = EnumToString(tfs[i]); StringReplace(lbl, "PERIOD_", "");

        string tf_str_only = tf_res;
        int colon_idx = StringFind(tf_res, ": ");
        if (colon_idx > 0) tf_str_only = StringSubstr(tf_res, colon_idx + 2);

        string uyum = "";
        if (direction != 0) {
            bool yatay = (StringFind(tf_str_only, "Yataya Bagladi") >= 0);
            bool yok = (StringFind(tf_str_only, "Kutu yok") >= 0);

            if (yok) {
                uyum = "⚪ Kutu Yok";
            } else if (yatay) {
                uyum = "⚠️ Yataya Bagladi";
            } else {
                bool supports_down = (StringFind(tf_str_only, "Asagi") >= 0 || StringFind(tf_str_only, "Alti Deldi") >= 0 || StringFind(tf_str_only, "Alttan Girdi") >= 0);
                bool supports_up = (StringFind(tf_str_only, "Yukari") >= 0 || StringFind(tf_str_only, "Yukari Deldi") >= 0 || StringFind(tf_str_only, "Ustten Girdi") >= 0);

                if (direction == -1) {
                    if (supports_down) uyum = "🟢 Yon Uyumlu";
                    else uyum = "🔴 Yon Uyumsuz";
                } else if (direction == 1) {
                    if (supports_up) uyum = "🟢 Yon Uyumlu";
                    else uyum = "🔴 Yon Uyumsuz";
                }
            }
        } else {
            bool yok = (StringFind(tf_str_only, "Kutu yok") >= 0);
            if (yok) uyum = "⚪ Kutu Yok";
            else uyum = "🔍 Kutu Incelendi";
        }
...
void SendTestNotif()
{
    string msg = BuildMTFNotification(0, 0, "");
    if(InpAlertPopup) Alert(msg);
    if(InpAlertPush)  SendNotification(msg);
                ...
                const long &volume[], const int &spread[])
{
    ...
    if(vp==0){
        ...
        // Anchor zamanından baslat - shadow analizi ile tutarli olsun
        int si=0;
    ...
    // Test bildirimi - InpNotifTest açıkken 1 kez gönder
