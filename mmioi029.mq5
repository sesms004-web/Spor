...
//====================================================================
//  MTF SİNYAL PANELİ (Comment)
//====================================================================

struct TFInfo { string name; ENUM_TIMEFRAMES tf; };

int GetGeneralTrend(ENUM_TIMEFRAMES tf)
{
   MqlRates r[];
   if(CopyRates(Symbol(),tf,1,5,r)<5) return 0;
   ArraySetAsSeries(r,true);
   return (r[0].close > r[4].close) ? 1 : -1;
}

string GetTFSignal(ENUM_TIMEFRAMES tf, string tf_name,
                   double bx_top, double bx_bot,
                   double wk_top, double wk_bot,
                   int &out_box_dir, int &out_gen_dir)
{
   out_box_dir=0; out_gen_dir=0;
   MqlRates r[];
   if(CopyRates(Symbol(),tf,0,28,r)<5) return "";
   ArraySetAsSeries(r,true);
   out_gen_dir = GetGeneralTrend(tf);

   int    hit_bar=-1, hit_appr=0, hit_state=0;
   double prev_c=r[27].close;

   for(int b=25;b>=1;b--)
   {
      double h=r[b].high, l=r[b].low, c=r[b].close;
      bool in_main=(h>=bx_bot)&&(l<=bx_top);
      bool in_wk_a=(h>=bx_top)&&(l<=wk_top)&&!in_main;
      bool in_wk_b=(h>=wk_bot)&&(l<=bx_bot)&&!in_main;

      if(in_main)
      {
         int appr=(prev_c>bx_top)?1:((prev_c<bx_bot)?-1:hit_appr);
         if(hit_bar<0){ hit_bar=b; hit_appr=appr; hit_state=1; }
         if(hit_appr==1  && l<wk_bot){ hit_state=2; hit_bar=b; break; }
         if(hit_appr==-1 && h>wk_top){ hit_state=2; hit_bar=b; break; }
      }
      else if(hit_state==1)
      {
         if(hit_appr==1  && c>bx_top) { hit_state=3; hit_bar=b; break; }
         if(hit_appr==-1 && c<bx_bot) { hit_state=3; hit_bar=b; break; }
   ...
   // Mum sayısı
   string t_ago=IntegerToString(hit_bar)+"mum";

   string box_tr, gen_tr, uyum, durum;
   gen_tr=(out_gen_dir==1)?"Yukari":"Asagi";

   if(hit_state==0) // zayıf bölge
   {
      out_box_dir=(hit_appr==1)?1:-1;
      box_tr=(hit_appr==1)?"LONG":"SHORT";
      durum=(hit_appr==1)?"Ust zayif, yaklasıyor":"Alt zayif, yaklasıyor";
   }
   else if(hit_state==1) // içinde
   {
      out_box_dir=(hit_appr==1)?1:-1;
      box_tr=(hit_appr==1)?"LONG":"SHORT";
      durum=(hit_appr==1)?"Ustunden icine girdi":"Altindan icine girdi";
   }
   else if(hit_state==2) // deldi
   {
      out_box_dir=(hit_appr==1)?-1:1;
      box_tr=(hit_appr==1)?"SHORT":"LONG";
      durum=(hit_appr==1)?"Alti deldı":"Ustu deldı";
   }
   else // bounce
   {
      out_box_dir=(hit_appr==1)?1:-1;
      box_tr=(hit_appr==1)?"LONG":"SHORT";
      durum=(hit_appr==1)?"Destek tuttu":"Direnc tuttu";
   }

   uyum=(out_box_dir==out_gen_dir)?"GUCLU":"KISA";
...
void DrawMTFSignalPanel()
{
   ENUM_TIMEFRAMES cur_tf=Period();
   string tf_name;
   switch(cur_tf)
   {
   ...
   {
      Comment("");
      sig = tf_name+" | Etkilesim yok\n \n ";
   }
   else
   {
      double bx_top=g_bx_top[best_k], bx_bot=g_bx_bot[best_k];
      double wk_top=g_bx_wk_abv_top[best_k], wk_bot=g_bx_wk_blw_bot[best_k];

      // Bu grafiğin kendi sinyalini hesapla
      int box_dir=0, gen_dir=0;
      sig=GetTFSignal(cur_tf, tf_name,
                              bx_top, bx_bot, wk_top, wk_bot,
                              box_dir, gen_dir);
      if(sig=="") sig=tf_name+" | Etkilesim yok\n \n ";
   }

   string parts[]; StringSplit(sig,'\n',parts);
   string L1=(ArraySize(parts)>0)?parts[0]:tf_name+" | ?";
   string L2=(ArraySize(parts)>1)?parts[1]:" ";
   string L3=(ArraySize(parts)>2)?parts[2]:" ";

   // Dosyaya yaz (M1 okuyacak)
   string fn="smacv2_sig_"+_Symbol+"_"+tf_name+".txt";
   int fh=FileOpen(fn,FILE_WRITE|FILE_TXT|FILE_COMMON|FILE_SHARE_READ);
   if(fh!=INVALID_HANDLE)
   {
      FileWriteString(fh,L1+"\n");
      FileWriteString(fh,L2+"\n");
      FileWriteString(fh,L3+"\n");
      FileClose(fh);
   }
   // Timestamp GlobalVariable'a yaz (Throttled)
   static datetime last_gv_update=0;
   if(TimeCurrent() - last_gv_update >= 5)
   {
      GlobalVariableSet("SMACv2_ts_"+_Symbol+"_"+tf_name,(double)TimeCurrent());
      last_gv_update = TimeCurrent();
   }

   // M1: sinyal değişince toplu bildirim gönder
   if(InpNotifSignal && cur_tf==PERIOD_M1)
      SendAnalysisNotif();

   if(cur_tf==PERIOD_M1)
   {
      // M1: tüm grafiklerin dosyalarını okuyup panel goster
      string tfs[7]={"M1","M5","M15","M30","H1","H4","D1"};
      int long_cnt=0,short_cnt=0,guclu_long=0,guclu_short=0;
      int lh=13, px=12, py=12, pw=340;
      int ph=7*(lh*3+6)+50;

      if(ObjectFind(0,"MTF_BG")<0) ObjectCreate(0,"MTF_BG",OBJ_RECTANGLE_LABEL,0,0,0);
      ObjectSetInteger(0,"MTF_BG",OBJPROP_CORNER,     CORNER_RIGHT_UPPER);
      ObjectSetInteger(0,"MTF_BG",OBJPROP_XDISTANCE,  px+pw);
      ...
      for(int i=0;i<7;i++)
      {
         string R1=tfs[i]+" | Grafige atili degil";
         string R2=" "; string R3=" ";

         ENUM_TIMEFRAMES tfp2[7]={PERIOD_M1,PERIOD_M5,PERIOD_M15,PERIOD_M30,
                                   PERIOD_H1,PERIOD_H4,PERIOD_D1};
         string gv_ts="SMACv2_ts_"+_Symbol+"_"+tfs[i];
         bool is_active=GlobalVariableCheck(gv_ts);

         if(is_active)
         {
            string f2="smacv2_sig_"+_Symbol+"_"+tfs[i]+".txt";
            int f=FileOpen(f2,FILE_READ|FILE_TXT|FILE_COMMON|FILE_SHARE_READ|FILE_SHARE_WRITE);
            if(f!=INVALID_HANDLE)
            {
               if(!FileIsEnding(f)) R1=FileReadString(f);
               if(!FileIsEnding(f)) R2=FileReadString(f);
               if(!FileIsEnding(f)) R3=FileReadString(f);
               FileClose(f);
            }
            else
            {
               R1=tfs[i]+" | Veri bekleniyor...";
            }
         }

         if(StringFind(R3,"GUCLU LONG") >=0){long_cnt++;guclu_long++;}
         else if(StringFind(R3,"KISA LONG")  >=0) long_cnt++;
         else if(StringFind(R3,"GUCLU SHORT")>=0){short_cnt++;guclu_short++;}
         else if(StringFind(R3,"KISA SHORT") >=0) short_cnt++;

         _MTFLabel("MTF_L1_"+IntegerToString(i),R1,px+pw,y,9); y+=lh;
         _MTFLabel("MTF_L2_"+IntegerToString(i),R2,px+pw,y,8); y+=lh;
         _MTFLabel("MTF_L3_"+IntegerToString(i),R3,px+pw,y,9); y+=lh+2;
         _MTFLabel("MTF_SP_"+IntegerToString(i),"---------------------------",px+pw,y,7); y+=lh;
      }

      string genel;
      if(long_cnt>short_cnt)
         genel=StringFormat("LONG BASKILI %d TF (%d GUCLU)",long_cnt,guclu_long);
      else if(short_cnt>long_cnt)
         genel=StringFormat("SHORT BASKILI %d TF (%d GUCLU)",short_cnt,guclu_short);
      else if(long_cnt==0&&short_cnt==0)
         genel="Etkilesim bekleniyor";
      else
         genel="KARMA | YON BELIRSIZ";

      _MTFLabel("MTF_SUM",genel,px+pw,y,9);
      Comment("");
      ChartRedraw(0);
   }
   else
   {
      // Diğer grafikler: paneli gizle, Comment'te kendi sinyalini goster
      if(ObjectFind(0,"MTF_BG")>=0) ObjectDelete(0,"MTF_BG");
      for(int i=0;i<7;i++)
      {
         if(ObjectFind(0,"MTF_L1_"+IntegerToString(i))>=0) ObjectDelete(0,"MTF_L1_"+IntegerToString(i));
         if(ObjectFind(0,"MTF_L2_"+IntegerToString(i))>=0) ObjectDelete(0,"MTF_L2_"+IntegerToString(i));
         if(ObjectFind(0,"MTF_L3_"+IntegerToString(i))>=0) ObjectDelete(0,"MTF_L3_"+IntegerToString(i));
         if(ObjectFind(0,"MTF_SP_"+IntegerToString(i))>=0) ObjectDelete(0,"MTF_SP_"+IntegerToString(i));
      }
      if(ObjectFind(0,"MTF_HDR")>=0) ObjectDelete(0,"MTF_HDR");
      if(ObjectFind(0,"MTF_SUM")>=0) ObjectDelete(0,"MTF_SUM");
      Comment("["+tf_name+"]\n"+L1+"\n"+L2+"\n"+L3);
   }
}

void _MTFLabel(string nm,string txt,int xd,int yd,int fsz)
{
...
// Toplu analiz bildirimi gönder (test veya otomatik)
void SendAnalysisNotif()
{
   if(Period()!=PERIOD_M1) return; // Sadece M1 gönderir
   ...
   ENUM_TIMEFRAMES tfp3[7]={PERIOD_M1,PERIOD_M5,PERIOD_M15,PERIOD_M30,
                             PERIOD_H1,PERIOD_H4,PERIOD_D1};
   for(int i=0;i<7;i++)
   {
      string R1=tfs[i]+" | Grafige atili degil";
      ...
      if(is_active)
      {
         ...
         {
            if(!FileIsEnding(f)) R1=FileReadString(f);
            if(!FileIsEnding(f)) FileReadString(f); // L2 atla
            if(!FileIsEnding(f)) R3=FileReadString(f);
            FileClose(f);
         }
         else
      ...
      notif    += R1+"\n"+(R3!=" "?R3+"\n":"")+"---\n";
      genel_key += tfs[i]+":"+R3+"|";
      if(StringFind(R3,"LONG") >=0) long_cnt++;
      if(StringFind(R3,"SHORT")>=0) short_cnt++;
   }

   string genel;
   if(long_cnt>short_cnt)      genel="LONG BASKILI "+IntegerToString(long_cnt)+" TF";
   else if(short_cnt>long_cnt) genel="SHORT BASKILI "+IntegerToString(short_cnt)+" TF";
   else                         genel="KARMA";

   string full="== SMACv2 ANALİZ ==\n"+notif+"GENEL: "+genel;
   if(genel_key != g_last_signal)
   {
      SendNotification(full);
      g_last_signal=genel_key;
   }
}
...
int OnInit()
{
   ...
   g_my_tf_name = tf_name;
   GlobalVariableSet("SMACv2_ts_"+_Symbol+"_"+tf_name,(double)TimeCurrent());

   // Test: grafiğe atınca mevcut tüm TF analizini gönder
   if(InpNotifTest) SendAnalysisNotif();
...
void OnDeinit(const int reason)
{
   ...
   {
      string gv_name = "SMACv2_ts_" + g_my_symbol + "_" + g_my_tf_name;
