   ...
   if (state.choch_dir == -1 && state.t2_h != 0 && state.d1_l != 0) {
      ...
      if (should_eval_bear && t2_valid) {
          ...
          if (InpShowChoch) {
              ...
              DrawLine(path_3, GetTimeSafe(time, state.t2_i), state.t2_h, time[i], state.d1_l, InpColorChochPath, 1, STYLE_DOT, false);
      ...
      if (should_eval_bull && t2_valid) {
          ...
          if (InpShowChoch) {
              ...
              string path_3 = GetUniqueName(prefix + "CHoCH_Path_");
              DrawLine(path_3, GetTimeSafe(time, state.t2_i), state.t2_l, time[i], state.d1_h, InpColorChochPath, 1, STYLE_DOT, false);
            ...
            if(draw_ui && InpShowMaj)
              {
               ...
               DrawLine(state.cur_top_line, GetTimeSafe(time, state.maj_h_i), state.maj_h, time[i] + PeriodSeconds(), state.maj_h, InpColorBull, 1, STYLE_DASH, true);

               if(state.maj_l != EMPTY_VALUE && state.maj_l != 0)
                 {
                  state.cur_bot_line = GetUniqueName(prefix + "HLine_Bot_");
                  DrawLine(state.cur_bot_line, GetTimeSafe(time, state.maj_l_i), state.maj_l, time[i] + PeriodSeconds(), state.maj_l, InpColorBull, 1, STYLE_DASH, true);
                 }
              }
           }
         ...
         if(act != EMPTY_VALUE && val_h > act)
           {
            ...
            if(draw_ui && InpShowMaj)
              {
               state.cur_bot_line = GetUniqueName(prefix + "HLine_Bot_");
               DrawLine(state.cur_bot_line, GetTimeSafe(time, state.maj_l_i), state.maj_l, time[i] + PeriodSeconds(), state.maj_l, InpColorBear, 1, STYLE_DASH, true);

               if(state.maj_h != EMPTY_VALUE && state.maj_h != 0)
                 {
                  state.cur_top_line = GetUniqueName(prefix + "HLine_Top_");
                  DrawLine(state.cur_top_line, GetTimeSafe(time, state.maj_h_i), state.maj_h, time[i] + PeriodSeconds(), state.maj_h, InpColorBear, 1, STYLE_DASH, true);
                 }
              }
           }
   ...
   if(InpShowMin)
     {
      ...
      DrawLine("LiveLeg", GetTimeSafe(time, g_state_curr.lp_i), g_state_curr.lp_p, time[leg_i], leg_p, InpColorMin, 1, STYLE_DOT);
     }
