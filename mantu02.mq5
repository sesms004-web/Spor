            ...
            if(!tv){state.choch_dir=0;state.d1_l=0;state.t2_h=0;}
                ...
                if(draw_ui){
                    ...
                    if(isn&&tdx>=0&&InpShowChoch){
                        ...
                        DrawLine(GetUniqueName(pfx+"CHoCH_Signal_"),GetTimeSafe(time,i),state.d1_l,GetTimeSafe(time,i)+PeriodSeconds()*5,state.d1_l,sc,is_strong ? 4 : 2,(tdx<=4)?STYLE_DASH:STYLE_SOLID);
                    }
            ...
            if(!tv){state.choch_dir=0;state.d1_h=0;state.t2_l=0;}
                ...
                if(draw_ui){
                    ...
                    if(isn&&tdx>=0&&InpShowChoch){
                        ...
                        DrawLine(GetUniqueName(pfx+"CHoCH_Signal_"),GetTimeSafe(time,i),state.d1_h,GetTimeSafe(time,i)+PeriodSeconds()*5,state.d1_h,sc,is_strong ? 4 : 2,(tdx<=4)?STYLE_DASH:STYLE_SOLID);
                    }
                    // ── Tum CHoCH'larda bildirim ──
