//+------------------------------------------------------------------+
//|                                                    Structure.mq5 |
//|       Minor + Major + CHoCH + Kutu + MTF İşlem Kararı            |
//+------------------------------------------------------------------+
...
input bool InpEnableM1  = true;
input bool InpEnableM3  = true;
input bool InpEnableM5  = true;
...
// CHoCH trade sayacı
double g_trade_t1_h[2]; double g_trade_t2_h[2]; int g_trade_count_h=0; int g_current_maj_h_i=0;
double g_trade_t1_l[2]; double g_trade_t2_l[2]; int g_trade_count_l=0; int g_current_maj_l_i=0;
int    g_last_notif_d1i_sell=-1; int g_last_notif_d1i_buy=-1;

void ResetBearishMemory(){g_trade_count_h=0;for(int i=0;i<2;i++){g_trade_t1_h[i]=0;g_trade_t2_h[i]=0;}}
void ResetBullishMemory(){g_trade_count_l=0;for(int i=0;i<2;i++){g_trade_t1_l[i]=0;g_trade_t2_l[i]=0;}}
...
bool IsApproved(int choch_dir, int ts, int appr, int break_up, int break_dn)
{
   ...
   bool yatay=(InpExhaustionCount>0)&&(break_up>=1)&&(break_dn>=1)&&(total>=InpExhaustionCount);
   if(yatay) return false;
   if(choch_dir==-1){
      if(ts==1&&appr==-1)return true; if(ts==2&&appr== 1)return true; if(ts==3&&appr==-1)return true;
   }else{
      if(ts==1&&appr== 1)return true; if(ts==2&&appr==-1)return true; if(ts==3&&appr== 1)return true;
   }
   return false;
...
string GetBoxStatusShort(int ts, int appr, int break_up, int break_dn)
{
   int total=break_up+break_dn;
   bool yatay=(InpExhaustionCount>0)&&(break_up>=1)&&(break_dn>=1)&&(total>=InpExhaustionCount);
   if(yatay) return StringFormat("YATAY(U:%d A:%d)",break_up,break_dn);
...
void BxAdd(string nm,string wk_abv,string wk_blw,double top,double bot)
{
   if(g_shadow_mode){
      ...
      g_shd_break_up[g_shd_cnt]=0; g_shd_break_dn[g_shd_cnt]=0;
      g_shd_cnt++; return;
   }
...
//=====================================================================
// MTF — TF Kutu Analizi (Shadow)
//=====================================================================
// Forward declaration — ProcessBar sonra tanımlanıyor
...
bool AnalyzeTFBoxResult(ENUM_TIMEFRAMES tf,double days_inp,TFBoxResult &res)
{
   ...
   res.cnt_in=0;res.ev_t=0;res.break_up=0;res.break_dn=0;
   ...
   for(int i=si+1;i<n-1;i++){
      ...
      if(!inside){
         ...
         ProcessBar(i,o,h,l,c,t,st,true);
...
void ResolveTFRow(const TFBoxResult &res,int choch_dir,
                  ...
                  bool &gec_out,bool &yat_out,bool &appr_out)
{
   ENUM_TIMEFRAMES tf=res.tf;int ps=PeriodSeconds(tf);
   bool m_ok=res.has_box&&(res.touch_state>0);
   bool m_gec=!m_ok||IsGecersiz(res.touch_state,res.cnt_in,res.ev_t,tf);
   int  total=res.break_up+res.break_dn;
   bool m_yat=(!m_gec)&&(InpExhaustionCount>0)&&(res.break_up>=1)&&(res.break_dn>=1)&&(total>=InpExhaustionCount);

   gec_out=m_gec;yat_out=m_yat;appr_out=false;

   if(m_gec){time_out="25+ mum      ";status_out="-";return;}

   if(res.touch_state==1) time_out=StringFormat("%d mum icinde  ",res.cnt_in);
   else{int mu=(ps>0)?(int)((TimeCurrent()-res.ev_t)/ps):0;time_out=StringFormat("%d mum once    ",mu);}

   status_out=GetBoxStatusShort(res.touch_state,res.approach,res.break_up,res.break_dn);
   if(!m_yat) appr_out=IsApproved(choch_dir,res.touch_state,res.approach,res.break_up,res.break_dn);
}

//=====================================================================
// SendChochNotif — DÜZELTİLMİŞ CASCADE
//=====================================================================
void SendChochNotif(int choch_dir,bool is_strong,double pb_pct,datetime choch_time,
                    ENUM_TIMEFRAMES signal_tf,int tdx)
{
   ENUM_TIMEFRAMES tfs[8]   ={PERIOD_M1,PERIOD_M3,PERIOD_M5,PERIOD_M15,PERIOD_M30,PERIOD_H1,PERIOD_H4,PERIOD_D1};
   double days_arr[8]       ={InpDaysM1,InpDaysM3,InpDaysM5,InpDaysM15,InpDaysM30,InpDaysH1,InpDaysH4,InpDaysD1};
   bool   ena[8]            ={InpEnableM1,InpEnableM3,InpEnableM5,InpEnableM15,InpEnableM30,InpEnableH1,InpEnableH4,InpEnableD1};

   TFBoxResult results[8];datetime ev_times[8];int count=0;
   for(int i=0;i<8;i++){
      if(!ena[i])continue;
      TFBoxResult res;AnalyzeTFBoxResult(tfs[i],days_arr[i],res);res.tf=tfs[i];
      results[count]=res;
      ev_times[count]=(res.has_box?res.ev_t:0);
      count++;
   }
   // En güncel etkileşim üste
   ...
   for(int i=0;i<count;i++)if(!row_gec[i]&&row_yat[i]){has_yatay=true;break;}

   // Gecersiz sayısı
   int n_gec=0;
   for(int i=0;i<count;i++)if(row_gec[i])n_gec++;

   // ── DÜZELTİLMİŞ CASCADE ─────────────────────────────────────────
   bool   trade_ok=false;
   string trade_reason="İşlem Yok";

   if(n_gec>2)
   {
      trade_reason=StringFormat("İşlem Yok (%d geçersiz TF)",n_gec);
   }
   else if(count>0 && !row_gec[0] && row_appr[0])
   {
      trade_ok=true;
      trade_reason="1. Kutu Onaylı → İşleme Gir";
   }
   else if(!has_yatay && count>=3 && !row_gec[1] && !row_gec[2] && row_appr[1] && row_appr[2])
   {
      // Her zaman sabit 2. ve 3. bakılır — atla yok
      trade_ok=true;
      trade_reason="2.+3. Kutu Onaylı → İşleme Gir";
   }
   else if(has_yatay && count>=4 && !row_gec[1] && !row_gec[2] && !row_gec[3]
           && row_appr[1] && row_appr[2] && row_appr[3])
   {
      // Yatay: sabit 2. + 3. + 4.
      trade_ok=true;
      trade_reason="2.+3.+4. Kutu Onaylı (Yatay) → İşleme Gir";
   }
   else if(has_yatay && !trade_ok)
   {
      trade_reason="İşlem Yok (Yatay - 3 onay gerekli)";
   }
   // ─────────────────────────────────────────────────────────────────
                ...
                const double &close[],const datetime &time[],SState &state,bool is_history)
{
   ...
   if(state.min_tr==1){
      ...
      if(val_l<ot){
         ...
         if(is_history&&InpShowBox&&state.maj_st==0&&state.maj_tr==1){
            ...
            {DoDrawBox(time,pfx,state,si,pp,sp,InpColorBoxBull);state.bx_phase=1;state.bx_extreme=false;state.bx_swing_h=pp;state.bx_swing_l=sp;}
      ...
      if(val_h>ot){
         ...
         if(is_history&&InpShowBox&&state.maj_st==0&&state.maj_tr==-1){
            if(state.bx_phase==0||state.bx_phase==2)
            {DoDrawBox(time,pfx,state,si,sp,tp,InpColorBoxBear);state.bx_phase=1;state.bx_extreme=false;state.bx_swing_h=sp;state.bx_swing_l=tp;}
   ...
   if(state.choch_dir==-1&&state.t2_h!=0&&state.d1_l!=0){
      if(val_c<state.d1_l&&in_pb){
         ...
         // Trade tracking + bildirim (sadece canlı bar, shadow değil)
   ...
   if(state.maj_tr==1){
      ...
      if(state.maj_st==0){
         ...
         {state.maj_l=val_l;if(InpShowMaj)UpdateLineLevel(state.cur_bot_line,state.maj_l);}
