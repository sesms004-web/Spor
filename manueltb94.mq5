      ...
      if(val_c<state.d1_l&&in_pb){
         ...
         if(!g_shadow_mode&&!is_history){
            ...
            if(vs || tdx >= 0) {
               ...
               if(sw > 0) {
                  // Sabit SL/TP (Ornek olarak Swing/4 ve 3R, kod ici ayarlar)
                  double sd = sw * 25.0 / 100.0; // 25% Stop loss
                  double rr = 3.0; // 3R Take profit
                  ...
                  if(!is_history && vs) { // Gonderim sarti (Sadece yeni canli islem)
...
%s",
                     ...
                     // TradeReceiver icin ortak dosyaya yazma
                     string file_name = "CHoCH_Signal_" + Symbol() + ".csv";
                     int h = FileOpen(file_name, FILE_WRITE | FILE_CSV | FILE_COMMON | FILE_ANSI, ',');
                     if(h != INVALID_HANDLE) {
                        FileWrite(h, Symbol(), "SELL", DoubleToString(entry, 5), DoubleToString(sl, 5), DoubleToString(tp, 5), TimeToString(t1));
                        FileClose(h);
                     }
                  }
               }
...
%s",
                     ...
                     int h = FileOpen(file_name, FILE_WRITE | FILE_CSV | FILE_COMMON | FILE_ANSI, ',');
                     if(h != INVALID_HANDLE) {
                        FileWrite(h, Symbol(), "BUY", DoubleToString(entry, 5), DoubleToString(sl, 5), DoubleToString(tp, 5), TimeToString(t1));
                        FileClose(h);
                     }
                  }
               }
...
//--- Kompakt panel - sag ust kose Cikarildi
//=====================================================================
